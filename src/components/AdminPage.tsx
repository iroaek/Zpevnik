...
export function AdminPage({ cloudSync, online, onNavigate, onOpenSong = () => undefined, songs = [], catalogVersion = '', downloadedSongs = 0, availableSongs = 0 }: { cloudSync: CloudSyncState; online: boolean; onNavigate: (path: string) => void; onOpenSong?: (id: string) => void; songs?: Song[]; catalogVersion?: string; downloadedSongs?: number; availableSongs?: number }) {
  ...
  return <section className="admin-page" aria-labelledby="admin-page-heading">
    <header className="admin-page-hero"><span><p className="eyebrow">Pro správce</p><h1 id="admin-page-heading">Správa zpěvníku</h1></span><Icon name="shield" /></header>
    <nav className="admin-tabs scroll-strip" role="tablist" aria-label="Sekce administrace">{labels.map(([value, label, description, icon]) => <button type="button" role="tab" id={`admin-tab-${value}`} aria-controls={`admin-panel-${value}`} key={value} className={tab === value ? 'active' : ''} aria-selected={tab === value} onClick={() => setTab(value)}><Icon name={icon} /><span><strong>{label}</strong><small>{description}</small></span></button>)}</nav>
    <div className="admin-tab-panel" role="tabpanel" id={`admin-panel-${tab}`} aria-labelledby={`admin-tab-${tab}`}>
