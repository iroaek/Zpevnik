...
export function OfflineContent({
  ...
  const downloadMemberLibrary = async () => {
    ...
    try {
      ...
      await onPersonalLibraryChanged?.();
      setNotice({ tone: 'success', text: `Hotovo: do tohoto zařízení bylo bezpečně uloženo ${count} ${secureProfile.role === 'admin' ? 'správcovských' : 'členských'} písní.` });
    } catch (error) {
    ...
    <section className="offline-page" aria-labelledby="offline-heading">
      ...
      <div className="offline-actions">
        {secureMode && <>
          ...
          <article className="member-library-download"><div><h3>{secureProfile?.role === 'admin' ? 'Správcovský balíček' : 'Členský balíček'}</h3><p>{downloadedLibrarySongs.length > 0 ? `${downloadedLibrarySongs.length} písní je uloženo v tomto zařízení a funguje bez internetu.` : 'V zařízení zatím nejsou stažené žádné členské písně.'}</p><small>Obnovení nahradí pouze předchozí členský balíček. Vaše vlastní PDF importy zůstanou zachované.</small></div><div className="offline-card-actions"><button className="primary-button" type="button" disabled={busy || !online || !secureProfile} onClick={() => void downloadMemberLibrary()}>{operation === 'member-library' ? 'Stahuji…' : downloadedLibrarySongs.length > 0 ? 'Obnovit knihovnu' : 'Stáhnout knihovnu'}</button>{downloadedLibrarySongs.length > 0 && <button className="danger-button" type="button" disabled={busy} onClick={() => setConfirmRemoveLibrary(true)}>Odstranit knihovnu</button>}</div></article>
        ...
        <div className="offline-section-heading"><span className="step-number" aria-hidden="true">{secureMode ? '2' : '1'}</span><span><p className="eyebrow">Volitelný obsah</p><h2>Ukázky a noty</h2></span></div>
