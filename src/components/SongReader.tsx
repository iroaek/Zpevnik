...
export function SongReader({ song, userState, onUserStateChange, onBack, catalogVersion, previousSong, nextSong, onPreviousSong, onNextSong, secureProfile = null }: SongReaderProps) {
  ...
  const settingsPresence = useAnimatedPresence(settingsOpen, 260);
  const correctionPresence = useAnimatedPresence(correctionOpen, 260);
  ...
  return (
    <article ref={readerRef} className={`song-reader song-reader--${readerSurface} song-reader--${performanceMode}`} onPointerDown={(event) => { revealStageControls(); if (event.pointerType === 'touch') swipeStart.current = { x: event.clientX, y: event.clientY }; }} onPointerUp={(event) => { const start = swipeStart.current; swipeStart.current = null; if (!start || event.pointerType !== 'touch' || stageLocked) return; const x = event.clientX - start.x; const y = event.clientY - start.y; if (Math.abs(x) < 70 || Math.abs(x) < Math.abs(y) * 1.5) return; haptic('selection'); if (x < 0) onNextSong?.(); else onPreviousSong?.(); }} onPointerCancel={() => { swipeStart.current = null; }}>
      ...
      <div className="reader-tabs" role="tablist" aria-label="Obsah písně">
        <button role="tab" aria-selected={tab === 'lyrics'} className={tab === 'lyrics' ? 'tab tab--active' : 'tab'} onClick={() => changeReaderTab('lyrics')}>Text a akordy</button>
        <button role="tab" aria-selected={tab === 'score'} className={tab === 'score' ? 'tab tab--active' : 'tab'} onClick={() => changeReaderTab('score')}>Noty ({song.scoreAssets.length})</button>
      </div>
        ...
        <>
          <section className="reader-toolbar" aria-label="Nastavení čtečky">
            ...
            <div className="toolbar-actions">
              ...
              <button type="button" className="icon-button" aria-label="Sdílet odkaz na píseň" onClick={() => void shareSong()}><Icon name="upload" /></button>
              <div className="performance-entry" role="group" aria-label="Režim hraní">
                <button type="button" className={`icon-button fire-button ${performanceMode === 'fire' ? 'active' : ''}`} aria-label="Režim u ohně" aria-pressed={performanceMode === 'fire'} onClick={() => void setPerformanceSurface('fire')}><Icon name="fire" /><span>U ohně</span></button>
                <button type="button" className={`icon-button stage-button ${performanceMode === 'stage' ? 'active' : ''}`} aria-label="Pódiový režim" aria-pressed={performanceMode === 'stage'} onClick={() => void setPerformanceSurface('stage')}><Icon name="expand" /><span>Pódium</span></button>
              </div>
            </div>
          </section>
          {!isLayoutText && <div className={`reader-segmented ${settings.showChords ? '' : 'reader-segmented--lyrics'}`} role="group" aria-label="Zobrazení textu"><span aria-hidden="true" /><button type="button" aria-pressed={settings.showChords} onClick={() => setChordVisibility(true)}>Akordy + text</button><button type="button" aria-pressed={!settings.showChords} onClick={() => setChordVisibility(false)}>Pouze text</button></div>}
