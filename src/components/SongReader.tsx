        ...
        <>
          ...
          {performanceActive && <>
            <div className={`stage-wake-actions ${stageControlsVisible ? 'stage-wake-actions--hidden' : ''}`} aria-hidden={stageControlsVisible}>
              <button type="button" className="stage-wake-button" aria-label={stageLocked ? 'Odemknout a zobrazit pódiové ovládání' : 'Zobrazit pódiové ovládání'} onClick={(event) => { event.stopPropagation(); flushSync(() => { if (stageLocked) setStageLocked(false); setStageControlsVisible(true); }); haptic('selection'); }}>{stageLocked ? '🔒' : '•••'}</button>
            ...
            <div className={`fire-dock performance-dock performance-dock--${performanceMode} ${stageControlsVisible ? '' : 'fire-dock--hidden'}`} aria-label={`Rychlé ovládání režimu ${performanceMode === 'fire' ? 'u ohně' : 'pódium'}`} aria-hidden={!stageControlsVisible}>
              ...
              <nav className="performance-dock__primary" aria-label="Hlavní ovládání hraní">
                ...
                <button type="button" aria-pressed={stageLocked} aria-label={stageLocked ? 'Odemknout ovládání' : 'Zamknout proti dotyku'} onClick={() => { setStageLocked((value) => !value); setStageControlsVisible(false); }}><span aria-hidden="true">{stageLocked ? '🔓' : '🔒'}</span><small>{stageLocked ? 'Odemknout' : 'Zamknout'}</small></button>
              </nav>
