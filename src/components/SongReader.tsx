...
import { calculateCapoOptions, parseChord, renderChord, transposeCanonicalChord, transposeChord, type CapoPlayerLevel } from '../domain/chords';
...
type PerformanceMode = 'off' | 'fire' | 'stage';
...
export function SongReader({ song, userState, onUserStateChange, onBack, catalogVersion, previousSong, nextSong, onPreviousSong, onNextSong, secureProfile = null }: SongReaderProps) {
  ...
  const [semitones, setSemitones] = useState(() => userState.songReaderPreferences[song.id]?.transpose ?? 0);
  ...
  const metronome = useMetronome(song.tempo ?? 90);
  ...
  const readerMobileColumnWidth = mobileColumnPercent(readerPreferences.columnWidth);
  const hasSongReaderPreferences = Boolean(songReaderPreferences);
  const readerScrollSpeed = songReaderPreferences?.autoScrollSpeed ?? settings.autoScrollSpeed;
  const isLayoutText = song.contentFormat === 'layout_text';
  const performanceActive = performanceMode !== 'off';
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 1) { setAutoScroll(false); return; }
      const elapsed = Math.min(now - previous, 80);
      previous = now;
      window.scrollBy({ top: readerScrollSpeed * elapsed / 1000, left: 0, behavior: 'auto' });
      frame = requestAnimationFrame(tick);
    };
    frame = requestAnimationFrame(tick);
    const pauseForInput = () => setAutoScroll(false);
    const pauseForKey = (event: KeyboardEvent) => { if (['ArrowDown', 'ArrowUp', 'PageDown', 'PageUp', 'Home', 'End', ' '].includes(event.key)) setAutoScroll(false); };
    window.addEventListener('wheel', pauseForInput, { passive: true });
    window.addEventListener('touchstart', pauseForInput, { passive: true });
    window.addEventListener('keydown', pauseForKey);
    return () => {
      window.removeEventListener('wheel', pauseForInput);
      window.removeEventListener('touchstart', pauseForInput);
      window.removeEventListener('keydown', pauseForKey);
      cancelAnimationFrame(frame);
      root.style.scrollBehavior = previousScrollBehavior;
    };
  }, [autoScroll, readerScrollSpeed]);

  useEffect(() => () => { void wakeLock?.release(); }, [wakeLock]);

  useEffect(() => {
    document.documentElement.dataset.fireMode = 'false';
    document.documentElement.dataset.performanceMode = 'off';
    return () => {
  ...
  }, [performanceActive, settingsOpen, stageControlsVisible, stageLocked, autoScroll]);
  ...
  const targetKey = useMemo(() => {
    ...
    return parsed ? renderChord(transposeCanonicalChord(parsed, semitones), settings.notation, parsed.root.accidental === 'flat' ? 'flat' : 'sharp') : song.originalKey;
  ...
  const readerFontSize = performanceMode === 'stage'
    ? Math.min(42, readerPreferences.stageFontSize + 2)
    : performanceMode === 'fire' ? readerPreferences.stageFontSize : settings.fontSize;
  ...
  const setChordVisibility = (showChords: boolean) => {
    if (settings.showChords === showChords) return;
    runElementTransition(readerRef.current, () => {
      flushSync(() => updateSettings({ showChords }));
    }, { name: 'reader-content', targetSelector: '.reader-performance-surface', duration: 220 });
    haptic('selection');
  };

  const changeReaderTab = (nextTab: 'lyrics' | 'score') => {
    if (tab === nextTab) return;
    runElementTransition(readerRef.current, () => {
      flushSync(() => setTab(nextTab));
    }, { name: 'reader-tab', duration: 240 });
  };
  ...
  const toggleSongReaderPreferences = (enabled: boolean) => {
    onUserStateChange((current) => {
      ...
      if (enabled) songPreferences[song.id] = {
        ...current.settings.reader,
        transpose: semitones,
        capoFret,
        autoScrollSpeed: current.settings.autoScrollSpeed,
      };
  ...
  const setSongTranspose = (value: number) => {
    const next = Math.max(-12, Math.min(12, value));
    setSemitones(next);
    if (hasSongReaderPreferences) updateReaderPreferences({ transpose: next });
  };

  const setSongCapo = (value: number) => {
    const next = Math.max(0, Math.min(11, value));
    setCapoFret(next);
    if (hasSongReaderPreferences) updateReaderPreferences({ capoFret: next });
  };

  const setReaderScrollSpeed = (value: number) => {
    const next = Math.max(5, Math.min(100, value));
    if (hasSongReaderPreferences) updateReaderPreferences({ autoScrollSpeed: next });
    else updateSettings({ autoScrollSpeed: next });
  };
  ...
  const setPerformanceSurface = async (requested: Exclude<PerformanceMode, 'off'>) => {
    const next: PerformanceMode = performanceMode === requested ? 'off' : requested;
    if (next !== 'off') {
      ...
      setAutoScroll(false);
      setCountdown(null);
      if (metronome.running) await metronome.toggle();
      if (wakeLock) await toggleWakeLock();
    }
    runElementTransition(readerRef.current, () => {
      document.documentElement.dataset.fireMode = next !== 'off' ? 'true' : 'false';
      document.documentElement.dataset.performanceMode = next;
      flushSync(() => setPerformanceMode(next));
    }, { name: 'performance', targetSelector: '.reader-performance-surface', duration: 280 });
    haptic('selection');
  };

  const revealStageControls = () => {
    if (!performanceActive || stageLocked) return;
    setStageControlsVisible(true);
  };

  const shareSong = async () => {
    const data = { title: song.title, text: `${song.title} — ${song.authors.join(', ') || 'Autor neuveden'}`, url: window.location.href };
    try {
      if (navigator.share) await navigator.share(data);
      else {
        await navigator.clipboard.writeText(`${data.text}\n${data.url}`);
        setSetlistMessage('Odkaz na píseň byl zkopírován.');
      }
      haptic('success');
    } catch (error) {
      if ((error as Error).name !== 'AbortError') setSetlistMessage('Sdílení se nepodařilo.');
    }
  };
  ...
  return (
    <article ref={readerRef} className={`song-reader song-reader--${readerSurface} song-reader--${performanceMode}`} onPointerDown={(event) => { revealStageControls(); if (event.pointerType === 'touch') swipeStart.current = { x: event.clientX, y: event.clientY }; }} onPointerUp={(event) => { const start = swipeStart.current; swipeStart.current = null; if (!start || event.pointerType !== 'touch' || stageLocked) return; const x = event.clientX - start.x; const y = event.clientY - start.y; if (Math.abs(x) < 70 || Math.abs(x) < Math.abs(y) * 1.5) return; haptic('selection'); if (x < 0) onNextSong?.(); else onPreviousSong?.(); }} onPointerCancel={() => { swipeStart.current = null; }}>
      ...
      <header className="reader-header">
        ...
        <div><h1 data-view-transition-target="song-title">{song.title}</h1><p>{song.authors.join(', ') || 'Autor neuveden'}</p></div>
      ...
      {performanceActive && <header className={`performance-song-bar ${stageControlsVisible ? '' : 'performance-song-bar--hidden'}`} aria-hidden={!stageControlsVisible}>
        <button type="button" aria-label="Zpět do seznamu" onClick={onBack}><Icon name="back" /></button>
        <span><small>{performanceMode === 'fire' ? 'U ohně' : 'Pódium'} · {targetKey ?? 'bez tóniny'}{capoFret ? ` · capo ${capoFret}` : ''}</small><strong>{song.title}</strong></span>
        <button type="button" aria-label={`Ukončit režim ${performanceMode === 'fire' ? 'u ohně' : 'pódium'}`} onClick={() => void setPerformanceSurface(performanceMode as Exclude<PerformanceMode, 'off'>)}><Icon name="close" /></button>
      </header>}

      <p className="reader-metadata"><span>Původní tónina: <strong>{song.originalKey ?? 'neuvedena'}</strong></span><span>Kapodastr: <strong>{capoFret ? capoFret + '. pražec' : 'bez'}</strong></span></p>
      <div className="reader-tabs" role="group" aria-label="Obsah písně">
        <button type="button" aria-pressed={tab === 'lyrics'} className={tab === 'lyrics' ? 'tab tab--active' : 'tab'} onClick={() => changeReaderTab('lyrics')}>Text a akordy</button>
        <button type="button" aria-pressed={tab === 'score'} className={tab === 'score' ? 'tab tab--active' : 'tab'} onClick={() => changeReaderTab('score')}>Noty ({song.scoreAssets.length})</button>
      </div>
        ...
        <>
          <section className="reader-toolbar" aria-label="Nastavení čtečky">
            {isLayoutText
              ...
              : <div className="transpose-control"><span className="visually-hidden">Transpozice</span><button type="button" aria-label="Snížit o půltón" disabled={semitones <= -12} onClick={() => { haptic(); setSongTranspose(semitones - 1); }}>−</button><span className="transpose-result"><strong aria-label="Výsledná tónina">{targetKey ?? "?"}</strong><output aria-label="Posun v půltónech">{semitones > 0 ? `+${semitones}` : semitones}</output></span><button type="button" aria-label="Zvýšit o půltón" disabled={semitones >= 12} onClick={() => { haptic(); setSongTranspose(semitones + 1); }}>+</button><button type="button" className="text-button" onClick={() => { haptic(); setSongTranspose(0); }}>Původní</button></div>}
            <div className="toolbar-actions"><button type="button" className="reader-settings-button secondary-button" aria-label="Otevřít nastavení zobrazení" onClick={() => setSettingsOpen(true)}>Aa <span>Zobrazení</span></button><button type="button" className="secondary-button" onClick={() => setActionsOpen(true)}><Icon name="menu" size={18} />Další</button>          {!performanceActive && <button type="button" className={`performance-fab ${autoScroll ? 'performance-fab--active' : ''}`} aria-label={autoScroll ? 'Pozastavit automatický posun' : countdown !== null ? 'Zrušit odpočet' : 'Spustit odpočet a automatický posun'} aria-pressed={autoScroll} onClick={performanceAction}><span aria-hidden="true">{countdown !== null ? countdown : <Icon name={autoScroll ? 'pause' : 'play'} size={23} />}</span><small>{autoScroll ? 'Pozastavit' : countdown !== null ? 'Zrušit' : 'Spustit posun'}</small></button>}</div>
          </section>
          ...
          {performanceActive && <>
            <div className={`stage-wake-actions ${stageControlsVisible ? 'stage-wake-actions--hidden' : ''}`} aria-hidden={stageControlsVisible}>
              <button type="button" className="stage-wake-button" aria-label={stageLocked ? 'Odemknout a zobrazit pódiové ovládání' : 'Zobrazit pódiové ovládání'} onClick={(event) => { event.stopPropagation(); flushSync(() => { if (stageLocked) setStageLocked(false); setStageControlsVisible(true); }); haptic('selection'); }}><Icon name={stageLocked ? 'lock' : 'menu'} /></button>
              <button type="button" className="stage-wake-button stage-exit-button" aria-label={performanceMode === 'fire' ? 'Ukončit režim u ohně' : 'Ukončit pódiový režim'} onClick={(event) => { event.stopPropagation(); void setPerformanceSurface(performanceMode as Exclude<PerformanceMode, 'off'>); }}><Icon name="close" /></button>
            </div>
            <div className={`fire-dock performance-dock performance-dock--${performanceMode} ${stageControlsVisible ? '' : 'fire-dock--hidden'}`} aria-label={`Rychlé ovládání režimu ${performanceMode === 'fire' ? 'u ohně' : 'pódium'}`} aria-hidden={!stageControlsVisible}>
              <div className="fire-stage-progress" role="progressbar" aria-label="Postup písně" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(readerProgress * 100)}><span><strong>{Math.round(readerProgress * 100)} %</strong><small>{performanceMode === 'fire' ? 'u ohně' : 'pódium'}</small></span><i><b style={{ width: `${Math.round(readerProgress * 100)}%` }} /></i></div>
              <nav className="performance-dock__primary" aria-label="Hlavní ovládání hraní">
                <button type="button" disabled={!previousSong} aria-label="Předchozí píseň" onClick={onPreviousSong}><Icon name="chevronLeft" /><small>Předchozí</small></button>
                <button type="button" className={`performance-dock__play ${autoScroll ? 'active' : ''}`} aria-label={autoScroll ? 'Pozastavit automatický posun' : countdown !== null ? 'Zrušit odpočet' : 'Spustit odpočet a posun'} onClick={performanceAction}><span>{countdown ?? <Icon name={autoScroll ? 'pause' : 'play'} />}</span><small>{autoScroll ? 'Pozastavit' : countdown !== null ? 'Zrušit' : 'Spustit posun'}</small></button>
                <button type="button" disabled={!nextSong} aria-label="Další píseň" onClick={onNextSong}><Icon name="chevronRight" /><small>Další</small></button>
                <button type="button" className={metronome.running ? 'active' : ''} aria-pressed={metronome.running} aria-label={metronome.running ? 'Vypnout metronom' : 'Zapnout metronom'} onClick={() => void metronome.toggle()}><span className={`metronome-beat ${metronome.running ? 'metronome-beat--active' : ''}`} data-beat={metronome.beat}>♩</span><small>{metronome.bpm} BPM</small></button>
                <button type="button" aria-label="Nastavení textu a výkonu" onClick={() => setSettingsOpen(true)}><strong>Aa</strong><small>Nastavit</small></button>
                <button type="button" aria-pressed={stageLocked} aria-label={stageLocked ? 'Odemknout ovládání' : 'Zamknout proti dotyku'} onClick={() => { setStageLocked((value) => !value); setStageControlsVisible(false); }}><Icon name={stageLocked ? 'unlock' : 'lock'} /><small>{stageLocked ? 'Odemknout' : 'Zamknout'}</small></button>
              </nav>
              <div className="performance-dock__adjustments">
                <div className="fire-font-control" role="group" aria-label="Velikost textu"><span>Text</span><button type="button" aria-label="Zmenšit text" disabled={readerPreferences.stageFontSize <= 14} onClick={() => updateReaderPreferences({ stageFontSize: Math.max(14, readerPreferences.stageFontSize - 2) })}>A−</button><output>{readerPreferences.stageFontSize} px</output><button type="button" aria-label="Zvětšit text" disabled={readerPreferences.stageFontSize >= 40} onClick={() => updateReaderPreferences({ stageFontSize: Math.min(40, readerPreferences.stageFontSize + 2) })}>A+</button></div>
                <label className="fire-speed-control"><span>Posun</span><input aria-label="Rychlost automatického posunu" type="range" min="5" max="100" value={readerScrollSpeed} onChange={(event) => setReaderScrollSpeed(Number(event.target.value))} /><output>{readerScrollSpeed}</output></label>
                <div className="performance-tempo-control" role="group" aria-label="Tempo metronomu"><button type="button" aria-label="Zpomalit metronom" onClick={() => metronome.setBpm(metronome.bpm - 1)}>−</button><button type="button" className="performance-tempo-control__tap" onClick={() => metronome.tap()}><strong>{metronome.bpm}</strong><small>Klepni tempo</small></button><button type="button" aria-label="Zrychlit metronom" onClick={() => metronome.setBpm(metronome.bpm + 1)}>+</button></div>
              </div>
            </div>
          </>}
          {countdown !== null && <div className="performance-countdown" role="status" aria-live="assertive" aria-label={`Start za ${countdown}`}><span>{countdown}</span><small>Připravit</small></div>}

          <Dialog open={actionsOpen} title="Další akce" onClose={() => setActionsOpen(false)}><div className="reader-more-actions">              {!isLayoutText && <button type="button" className={`icon-button chord-edit-toggle ${chordEditMode ? 'active' : ''}`} aria-label={chordEditMode ? 'Ukončit ruční posun akordů' : 'Ručně posunout akordy'} aria-pressed={chordEditMode} onClick={() => { setActionsOpen(false); setChordEditMode((value) => !value); setChordEditMessage(''); setAutoScroll(false); }}><Icon name="edit" /><span>Posunout akordy</span></button>}
              <button type="button" className="icon-button" aria-label="Nahlásit nebo lokálně opravit píseň" onClick={() => { setActionsOpen(false); openCorrection(); }}><Icon name="flag" /><span>Opravit píseň</span></button>
              <button type="button" className="icon-button" aria-label="Celoobrazovkový režim" onClick={() => { setActionsOpen(false); toggleFullscreen(); }}><Icon name="expand" /><span>Celá obrazovka</span></button>
              <button type="button" className="icon-button" aria-label="Sdílet odkaz na píseň" onClick={() => void shareSong()}><Icon name="upload" /><span>Sdílet odkaz</span></button>
</div>          <section className="field-actions" aria-label="Funkce pro zpívání">
