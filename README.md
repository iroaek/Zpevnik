...
Nasaďte beze změny celý adresář `dist/` na statický HTTPS hosting. Workflow `.github/workflows/deploy-pages.yml` při každém pushi ověří testy a přímo publikuje úspěšný produkční build na GitHub Pages. Příklad pro Cloudflare Pages je v `deploy/cloudflare-pages.example.json`. Přesný postup je v [DEPLOYMENT.md](DEPLOYMENT.md).
